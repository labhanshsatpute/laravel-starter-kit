...
@section('panel-header')
    <div>
        <ul class="breadcrumb">
            ...
            <li><i data-feather="chevron-right"></i></li>
            <li><a href="{{ route('admin.view.application.setting') }}">Application Settings</a></li>
        </ul>
        <h1 class="panel-title">Application Settings</h1>
    </div>
@endsection

@section('panel-body')
    <form action="{{route('admin.handle.application.setting')}}" method="POST" enctype="multipart/form-data">
        @csrf
        <figure class="panel-card">
            <div class="panel-card-header">
                <div>
                    <h1 class="panel-card-title">System Setting</h1>
    ...
    <script>
        ...
        const handleLogoPreview = (event) => {
            if (event.target.files.length == 0) {
                document.getElementById('profile').src = "{{ asset('storage/' . auth()->user()->profile_image) }}";
            } else {
